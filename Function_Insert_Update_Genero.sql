--Procedimiento que inserta un dato en la tabla Genero por primera vez, con todos los datos necesarios
create or replace procedure insertGenero(vcNombre in genero.nombre%type)is 
    BEGIN
        SAVEPOINT sp_safe;
        --Punto de seguridad por si pasa un error
        insert into genero(id_genero, nombre)
        VALUES(s_genero.nextval, vcNombre);
        COMMIT;
    EXCEPTION
